...
label_version = ttk.Label(root,text="       v0.3.2\n20220714",bootstyle="secondary")
...
def Run():
    ...
    if run_status.get()==0:
        ...
        os.system('start pythonw '+file_full_path+'\\bin\\autogui2.py')
        run_status.set(1)
        ...
        print("show_info: ",Messagebox.show_info(message=" AI-Aiming已启动\n 首次使用或切换模型需下载模型文件,请稍等\n > 按【X】键自动瞄准并射击",title="Tips",alert=False))  
    else:
        filename=file_full_path+'\\bin\\running_status.json'
        run_status.set(0)
        with open(filename,'w') as file_obj:
